 ...
 * @param {Array} otp - This method takes an array of otp/number as input.
 ...
 */
 const validateOtp = (messages, isOptional, length) => {
    ...
    return messages.map(name => {
        const validationChain = check(name, `Enter a valid ${name}`).isNumeric().isLength(length);
